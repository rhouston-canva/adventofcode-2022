...
fn parse_moves(s: &str) -> Result<Vec<Move>, ParseError> {
	...
	for line in s.lines() {
		...
		let this_move = Move {
			count: words[1].parse::<i32>().map_err(|_| ParseError)?,
			source: words[3].parse::<i32>().map(|v| (v - 1) as usize).map_err(|_| ParseError)?,
			target: words[5].parse::<i32>().map(|v| (v - 1) as usize).map_err(|_| ParseError)?,
		};
...
fn apply_move(stacks: &mut Stacks, the_move: Move) {
	for _ in 0..the_move.count {
		let item = stacks[the_move.source].pop()
			.expect("Tried to pop empty stack");
		stacks[the_move.target].push(item);
	}
}
